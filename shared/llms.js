...
(function () {
  ...
  // --- Inject site nav ---
  // Looks for .site-chrome (present on every page) and injects the
  // nav before it. Pages don't need any nav HTML — just the toggle placeholder.

  var toggleContainer = document.querySelector('.site-chrome');
  if (toggleContainer && !document.querySelector('.site-nav')) {
    ...
    toggleContainer.innerHTML =
      ...
      '<button class="icon-btn" id="theme-btn" title="Toggle light/dark mode" aria-label="Toggle theme">' +
        '<svg id="icon-sun" width="16" height="16" viewBox="0 0 16 16" fill="none" style="display:none">' +
        ...
        '</svg>' +
        '<svg id="icon-moon" width="16" height="16" viewBox="0 0 16 16" fill="none">' +
